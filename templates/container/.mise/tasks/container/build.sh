...
podman build --format docker --isolation chroot --ulimit nofile=65536:65536 --cache-from "${IMAGE_NAME}" \
	-t "${IMAGE_NAME}:${COMMIT_SHA}" --label "org.opencontainers.image.source=${REPO_URL}" \
	--label "org.opencontainers.image.title=${REPO_NAME}" --label "org.opencontainers.image.vendor=${REPO_OWNER}" \
	./src
